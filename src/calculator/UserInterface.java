...
/**
 * <p> Title: This is a Double Calculator. </p>
 * 
 ...
 * 
 * @author Sparsh Goel
 * @version 4.03	2018-02-25 The JavaFX-based GUI for the implementation of a calculator
 * 
 */
public class UserInterface {
	...
	**********************************************************************************************/
	...
	private TextField text_ResultMeasuredValue = new TextField();
	...
	private Button button_Sqt = new Button("\u221A");
	...
	**********************************************************************************************/
	...
	public UserInterface(Pane theRoot) {
		...
		setupTextUI(text_Operand1MeasuredValue, "Arial", 18, 450, Pos.BASELINE_LEFT, 10, 70, true);
		...
		setupTextUI(text_Operand1ErrorTerm, "Arial", 18, 150, Pos.BASELINE_LEFT, 
				Calculator.WINDOW_WIDTH/2 + 150, 70, true);
		...
		setupTextUI(text_Operand2MeasuredValue, "Arial", 18, 450, Pos.BASELINE_LEFT, 10, 165, true);
		...
		setupTextUI(text_Operand2ErrorTerm, "Arial", 18, 150, Pos.BASELINE_LEFT, 
				Calculator.WINDOW_WIDTH/2 + 150, 165, true);
		...
		text_Operand2ErrorTerm.setOnAction((event) -> { text_ResultMeasuredValue.requestFocus(); });
		...
		setupTextUI(text_ResultMeasuredValue, "Arial", 18, 450, Pos.BASELINE_LEFT, 10, 250, false);
		
		setupTextUI(text_ResultErrorTerm, "Arial", 18, 150, Pos.BASELINE_LEFT, Calculator.WINDOW_WIDTH/2 + 150, 250, false);
		...
		setupButtonUI(button_Add, "Symbol", 32, BUTTON_WIDTH, Pos.BASELINE_LEFT, 0.5 * buttonSpace-BUTTON_OFFSET, 300);
		...
		setupButtonUI(button_Sub, "Symbol", 32, BUTTON_WIDTH, Pos.BASELINE_LEFT, 1.5 * buttonSpace-BUTTON_OFFSET, 300);
		...
		setupButtonUI(button_Mpy, "Symbol", 32, BUTTON_WIDTH, Pos.BASELINE_LEFT, 2.5 * buttonSpace-BUTTON_OFFSET, 300);
		...
		setupButtonUI(button_Div, "Symbol", 32, BUTTON_WIDTH, Pos.BASELINE_LEFT, 3.5 * buttonSpace-BUTTON_OFFSET, 300);
		button_Div.setOnAction((event) -> { divOperands(); });
		
		// Code for square root button
		setupButtonUI(button_Sqt, "Symbol", 32, BUTTON_WIDTH, Pos.BASELINE_LEFT, 4.5 * buttonSpace-BUTTON_OFFSET, 300);
		button_Sqt.setOnAction((event) -> { sqtOperands(); });
		
		// Place all of the just-initialized GUI elements into the pane
		theRoot.getChildren().addAll(label_IntegerCalculator, label_Operand1, text_Operand1MeasuredValue, text_Operand1ErrorTerm, label_errOperand1, 
				label_Operand2, text_Operand2MeasuredValue, text_Operand2ErrorTerm, label_errOperand2, label_Result, text_ResultMeasuredValue, text_ResultErrorTerm, label_errResult, 
				button_Add, button_Sub, button_Mpy, button_Div, button_Sqt);
	...
	**********************************************************************************************/
	...
	private void setOperand1MeasuredValue() {
		text_ResultMeasuredValue.setText("");								// Any change of an operand probably invalidates
		label_Result.setText("Result");						// the result, so we clear the old result.
		label_errResult.setText("");
		if (perform.setOperand1MeasuredValue(text_Operand1MeasuredValue.getText())) {	// Set the operand and see if there was an error
			label_errOperand1.setText("");					// If no error, clear this operands error
			if (text_Operand2MeasuredValue.getText().length() == 0)		// If the other operand is empty, clear its error
	...
	private void setOperand1ErrorTerm() {
		...
		label_errResult.setText("");
		if (perform.setOperand1ErrorTerm(text_Operand1ErrorTerm.getText())) {	
			label_errOperand1.setText("");					
			if (text_Operand2MeasuredValue.getText().length() == 0)		
				label_errOperand2.setText("");				
		}
		else 												
			label_errOperand1.setText(perform.getOperand1ErrorMessage());	
	}
	...
	private void setOperand2MeasuredValue() {
		text_ResultMeasuredValue.setText("");								// See setOperand1's comments. The logic is the same!
		label_Result.setText("Result");				
		label_errResult.setText("");
		if (perform.setOperand2MeasuredValue(text_Operand2MeasuredValue.getText())) {
			label_errOperand2.setText("");
	...
	private void setOperand2ErrorTerm() {
		...
		label_Result.setText("Result");				
		label_errResult.setText("");
		if (perform.setOperand2ErrorTerm(text_Operand2ErrorTerm.getText())) {
			label_errOperand2.setText("");
	...
	private void addOperands(){
		...
		String theAnswer = perform.additionMeasuredValue();							// Call the business logic add method
		...
		if (theAnswer.length() > 0) {									// Check the returned String to see if it is okay
			text_ResultMeasuredValue.setText(theAnswer);				// If okay, display it in the result field and
			label_Result.setText("Sum");								// change the title of the field to "Sum"
		}
		else {															// Some error occurred while doing the addition.
			text_ResultMeasuredValue.setText("");						// Do not display a result if there is an error.				
	...
	private void subOperands(){
				...
				if (binaryOperandIssues()) 										// If there are issues with the operands, return
					return;														// without doing the computation
																				...
																				// If the operands are defined and valid, request the business logic method to do the addition and return the
																				// result as a String. If there is a problem with the actual computation, an empty string is returned
				String theAnswer = perform.subtractionMeasuredValue();						// Call the business logic add method
				label_errResult.setText("");									// Reset any result error messages from before
				if (theAnswer.length() > 0) {									// Check the returned String to see if it is okay
					text_ResultMeasuredValue.setText(theAnswer);								// If okay, display it in the result field and
					label_Result.setText("Difference");								// change the title of the field to "Sum"
				}
				else {															// Some error occurred while doing the addition.
					text_ResultMeasuredValue.setText("");									// Do not display a result if there is an error.				
					label_Result.setText("Result");								// Reset the result label if there is an error.
	...
	private void mpyOperands(){
		...
		if (binaryOperandIssues()) 											// If there are issues with the operands, return
			return;															// without doing the computation
																			...
																			// If the operands are defined and valid, request the business logic method to do the addition and return the
																			// result as a String. If there is a problem with the actual computation, an empty string is returned
		String theAnswer = perform.multiplicationMeasuredValue();						// Call the business logic add method
		label_errResult.setText("");										// Reset any result error messages from before
		if (theAnswer.length() > 0) {										// Check the returned String to see if it is okay
			text_ResultMeasuredValue.setText(theAnswer);									// If okay, display it in the result field and
			label_Result.setText("Product");								// change the title of the field to "Sum"
		}
		else {																// Some error occurred while doing the addition.
			text_ResultMeasuredValue.setText("");										// Do not display a result if there is an error.				
			label_Result.setText("Result");									// Reset the result label if there is an error.
	...
	private void divOperands(){
				...
				if (binaryOperandIssues()) 										// If there are issues with the operands, return
					return;														// without doing the computation
				double x = Double.parseDouble(text_Operand2MeasuredValue.getText());
				if (x==0) {
					label_errResult.setText("/ by zero is not allowed");
					text_ResultMeasuredValue.setText("");
					
				}
				else {String theAnswer = perform.divisionMeasuredValue();					// Call the business logic add method
				label_errResult.setText("");									// Reset any result error messages from before
				if (theAnswer.length() > 0) {									// Check the returned String to see if it is okay
					text_ResultMeasuredValue.setText(theAnswer);								// If okay, display it in the result field and
	...
	private void sqtOperands(){
																		// Check to see if both operands are defined and valid
		if (unaryOperandIssues()); 										// If there are issues with the operands, return														
																		// without doing the computation
		
		if (text_Operand2MeasuredValue.getLength() != 0) {
			text_Operand2MeasuredValue.setText("");
			label_errOperand2.setText("Second Input not Required.");
			
		}
																		// If the operands are defined and valid, request the business logic method to do the addition and return the
																		// result as a String. If there is a problem with the actual computation, an empty string is returned
		String theAnswer = perform.squareRootMeasuredValue();						// Call the business logic add method
		label_errResult.setText("");									// Reset any result error messages from before
		if (theAnswer.length() > 0) {									// Check the returned String to see if it is okay
			text_ResultMeasuredValue.setText(theAnswer);								// If okay, display it in the result field and
